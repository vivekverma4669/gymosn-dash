...
interface TrainerFormDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (input: CreateTrainerInput) => Promise<void>;
}

const emptyForm: CreateTrainerInput = { name: '', email: '', password: '' };

export const TrainerFormDialog: React.FC<TrainerFormDialogProps> = ({ isOpen, onClose, onSave }) => {
  const [form, setForm] = useState<CreateTrainerInput>(emptyForm);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-foreground">Add Trainer</h3>
                <p className="text-xs text-muted-foreground">Creates a login for this trainer.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="sana@apexfitness.com"
                  className="w-full rounded-xl border border-border bg-background px-3.5 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-hidden focus:ring-2 focus:ring-primary/20"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-border/60">
                ...
                >
                  {isSubmitting ? 'Creating...' : 'Add Trainer'}
                </button>
              </div>
            </form>
          </motion.div>
